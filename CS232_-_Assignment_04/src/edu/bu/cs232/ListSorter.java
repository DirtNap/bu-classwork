...
/**
 * @author dirtnap Abstract class providing a selection of sort types for
 *         sorting an array of {@link ShoppingListItem} elements. Unlike
 *         traditional implementations, these sort methods <i>can</i> sort
 *         sparse or empty arrays. <code>null</code> elements will sort last,
 *         and move to the end of the array.
 * 
 * @see SortProvider
 */
public abstract class ListSorter implements SortProvider<ShoppingListItem> {
    ...
    public void quickSort(ShoppingListItem[] theList) {
        ...
        if (theList.length > 2) {
            ...
            // This is a special case, because there is no pivot (no element is
            // "between" 2 elements.)
    ...
    public void mergeSort(ShoppingListItem[] theList) {
        class MergeSort {
            ...
            public void sort(ShoppingListItem[] theList) {
                ...
                this.number = this.inputList.length;
                this.swap = new ShoppingListItem[this.number];
    ...
    // Abstract Methods
    /**
     * Sorts the array using one of the concrete sorting methods provided by
     * this class. The particular sorting method is left to the concrete class.
     * 
     * @param theList
     *            the ShoppingListItem array to be sorted
     */
    @Override
