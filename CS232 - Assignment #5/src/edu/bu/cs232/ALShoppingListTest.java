...
public class ALShoppingListTest {
	...
	public void setUp() throws Exception {
		...
		for (int i = 0; i < this.testSLIArray.length; ++i) {
			...
			ShoppingListItem testItem = new ShoppingListItem(String.format("Test Item %d", i), priority, i);
	...
	public final void testGet() {
		for (int i = 0; i < this.testSLIArray.length; ++i) {
			...
			assertEquals(this.testSLIArray[i], this.testALShoppingList.get(this.testSLIArray[i].getName()));
		}
	}
	...
	public final void testDoSorting() {
		this.testALShoppingList.doSorting(sortedSLIArray);
	}
